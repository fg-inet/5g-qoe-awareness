...
#include "TCPLiveVideoStreamCliApp.h"
...
//Register_Class(TCPLiveVideoStreamCliApp);
Define_Module(TCPLiveVideoStreamCliApp);

simsignal_t TCPLiveVideoStreamCliApp::rcvdPkSignal = registerSignal("rcvdPk");
simsignal_t TCPLiveVideoStreamCliApp::sentPkSignal = registerSignal("sentPk");

simsignal_t TCPLiveVideoStreamCliApp::positionX = registerSignal("positionX");
simsignal_t TCPLiveVideoStreamCliApp::positionY = registerSignal("positionY");

TCPLiveVideoStreamCliApp::~TCPLiveVideoStreamCliApp() {
...
int TCPLiveVideoStreamCliApp::generateRandomNumberFromRange(int min, int max) {
    int mean = (max + min) / 2;
...
int TCPLiveVideoStreamCliApp::generateRandomizedBitrate(int minBitrate, int maxBitrate) {
...
int TCPLiveVideoStreamCliApp::getVideoBitrate(int resolution) {
    if (resolution == 240) {
...
int TCPLiveVideoStreamCliApp::getMaxBitrate(int resolution) {
    if (resolution == 240) {
        return 115;
    } else if (resolution == 360) {
        return 335;
...
void TCPLiveVideoStreamCliApp::nextVidSetup() {
...
void TCPLiveVideoStreamCliApp::initialize(int stage) {
...
void TCPLiveVideoStreamCliApp::sendRequest() {
...
void TCPLiveVideoStreamCliApp::scheduleVideoPlayEvent() {
    simtime_t d = simTime();
    if (videoType == "live") {
        simtime_t currPlaybackDelay = simTime() - refVidStartTime - video_playback_pointer;
        emit(DASH_live_delay_signal, currPlaybackDelay);
...
void TCPLiveVideoStreamCliApp::handleTimer(cMessage *msg) {
...
void TCPLiveVideoStreamCliApp::socketEstablished(TcpSocket *socket) { //M: When the socked has been established, send the first request
...
void TCPLiveVideoStreamCliApp::rescheduleOrDeleteTimer(simtime_t d, short int msgKind) { //M: Not entirely sure how to describe what is happening here. It probably has something to do with the lifetime of the video client in the simulation
...
double TCPLiveVideoStreamCliApp::calculateEWMA(std::list<double> list, double alpha) {
    if (list.size() == 1) {
        return list.front();
    } else {
...
void TCPLiveVideoStreamCliApp::socketDataArrived(TcpSocket *socket, Packet *msg, bool urgent) {
...
std::string TCPLiveVideoStreamCliApp::getVResString(int resolution) {
    if (resolution == 240) {
        return "426x240";
    } else if (resolution == 360) {
        return "640x360";
...
void TCPLiveVideoStreamCliApp::prepareCSV(void) {
    std::fstream fout;
...
double TCPLiveVideoStreamCliApp::calculateMOS(void) {
    // Prepare csv file used for MOS calculation
    TCPLiveVideoStreamCliApp::prepareCSV(); // Will prepare a csv file with the collected information
...
void TCPLiveVideoStreamCliApp::socketClosed(TcpSocket *socket) {
    EV << "Socket closed. Connection id: " << socket->getSocketId() << "\n";   //EDIT!
    TcpAppBase::socketClosed(socket);                      //EDIT!
}

void TCPLiveVideoStreamCliApp::socketFailure(TcpSocket *socket, int code) {
...
void TCPLiveVideoStreamCliApp::refreshDisplay() const
{
    char buf[64];
