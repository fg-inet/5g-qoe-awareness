...
def simpleAdmission(availBand, desiredQoE, cliTypes, maxNumCliType, desiredQoEceil, guaranteeMultiplier):
    usedBand = 0
    numHostsPerType = {}
    reqBitratesPerType = {}
    ...
    for host in cliTypes:
        if host == 'hostFDO':
            ...
            print('For a QoE of', desiredQoE, host, 'needs', reqBitratesPerType[host],'; The ceil for host is set accortding to QoE of', desiredQoE, 'and is equal to', ceilBitrates[host])
        else:
            ceilBitrates[host] = getBandForQoECli(host, desiredQoEceil)
            print('For a QoE of', desiredQoE, host, 'needs', reqBitratesPerType[host],'; The ceil for host is set accortding to QoE of', desiredQoEceil, 'and is equal to', ceilBitrates[host])
...
def genAllSliConfigsHTBRun(configName, baseName, availBand, desiredQoE, types, hostToSlice, sliceNames, maxNumCliType, desiredQoEceil, guaranteeMultiplier, differentiatePrios):
    cliTypes = ['host'+x for x in types]
    serverTypes = ['server'+x for x in types]
    numHostsPerType, reqBitratesPerType, ceilBitrates = simpleAdmission(availBand*1000, desiredQoE, ['host'+x for x in types], maxNumCliType, desiredQoEceil, guaranteeMultiplier)
    hostIPprefixes = {}
    serverIPprefixes = {}
    leafClassesConfigs = {}
    innerClassConfigs = {}
    ...
    genBaselineIniConfig(configName, baseName, numHostsPerType, hostIPprefixes, availBand, 900 + desiredQoEceil, guaranteeMultiplier) # The +900 indicates that the scenario uses a QoE range to determine the ceil rate. It's acturally not too important here

    f2 = open('../5gNS/simulations/runCommandsV2.txt', 'a+')
    f2.write('./runAndExportSimConfig.sh -i htbSimpleTestLite.ini -c ' + configName + ' -s 1\n')
    f2.close()


def prepareAllConfigs(testName, targetQoEs, assuredMultis, rates, maxCliPerRate, ceilQoEincrement):
    dp = 'False'
    for rate, maxCli in zip(rates, maxCliPerRate):
        for qoE in targetQoEs:
            for mult in assuredMultis:
                for ceilQoEinc in ceilQoEincrement:
                    genAllSliConfigsHTBRun(testName + 'No1Base_R'+str(int(rate))+'_Q'+str(int(qoE*10))+'_M'+str(int(mult*100))+'_C'+str(int(900+(ceilQoEinc+qoE)*10))+'_P'+str(dp), 'liteCbaselineTestTokenQoS_base', rate, qoE, ['VID', 'LVD', 'FDO', 'VIP', 'SSH'], [['VID', 'LVD', 'FDO', 'VIP', 'SSH']], ['connFIX0'], maxCli, qoE + ceilQoEinc, mult, dp)
                    genAllSliConfigsHTBRun(testName + 'No2_2sli_R'+str(int(rate))+'_Q'+str(int(qoE*10))+'_M'+str(int(mult*100))+'_C'+str(int(900+(ceilQoEinc+qoE)*10))+'_P'+str(dp), 'liteCbaselineTestTokenQoS_base', rate, qoE, ['VID', 'LVD', 'FDO', 'VIP', 'SSH'], [['VID', 'LVD', 'FDO'], ['VIP', 'SSH']], ['connBWS', 'connDES'], maxCli, qoE + ceilQoEinc, mult, dp)
                    genAllSliConfigsHTBRun(testName + 'No3_5sli_R'+str(int(rate))+'_Q'+str(int(qoE*10))+'_M'+str(int(mult*100))+'_C'+str(int(900+(ceilQoEinc+qoE)*10))+'_P'+str(dp), 'liteCbaselineTestTokenQoS_base', rate, qoE, ['VID', 'LVD', 'FDO', 'VIP', 'SSH'], [['VID'], ['LVD'], ['FDO'], ['VIP'], ['SSH']], ['connVID', 'connLVD', 'connFDO', 'connVIP', 'connSSH'], maxCli, qoE + ceilQoEinc, mult, dp)

prepareAllConfigs('expQoeAdmission40msQoErange', [3.0,3.5,4.0], [1.0], [100], [50], [0.5])
